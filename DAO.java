...
import yohx.DAO.FieldData;
import yohx.DAO.TableMetadata;
...
public class DAO {
    ...
    public DAO(String username, String password, String database, String sgbd){
    ...
    public <T> T insert(Connection con, T inserted) throws Exception {
        ...
        if (metadata.getId() != null) {
            last = getLastInsertedRow(con, metadata);
        } else {
            last = inserted;
        }
        if (opened) {
        ...
        public <T> List<T> select(Connection con, Object inserted) throws Exception{
            ...
            select_query=metadata.getFieldPredicates(select_query);
            System.out.println(select_query);
            Statement state=con.createStatement();
            ResultSet res=state.executeQuery(select_query);
            while (res.next()) {
                Object newObj=metadata.fillObject(res);
                all.add((T)newObj);
            }
            if (opened==true) {
                con.close();
    ...
    public <T> T getLastInsertedRow(Connection con, TableMetadata metadata) throws Exception{
        boolean opened=false;
        if (con==null) {
            con=this.connect();
            opened=true;
